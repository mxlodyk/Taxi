...
def iterative_deepening_a_star_search(problem):
    ...
    def dfs(state, g, threshold, path, visited):
        ...
        if problem.check_goal_state(state):
            return path, f
        # Track largest 'f' value that exceeds current threshold.
        # Largest value is used instead of smallest for reward maximisation.
        ...
        for next_state, action, reward in problem.successors(state):
            ...
            if next_state in visited:
                # Skip state if state has been visited.
                continue
            ...
            # Update 'max_threshold' if 'f' value from recursive call is larger.
